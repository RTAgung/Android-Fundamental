...
class MyPreferenceFragment : PreferenceFragmentCompat(),
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        sharedPreferences?.let {
            if (key == NAME) namePreference.summary = it.getString(NAME, DEFAULT_VALUE)
            if (key == EMAIL) emailPreference.summary = it.getString(EMAIL, DEFAULT_VALUE)
            if (key == AGE) agePreference.summary = it.getString(AGE, DEFAULT_VALUE)
            if (key == PHONE) phonePreference.summary = it.getString(PHONE, DEFAULT_VALUE)
            if (key == LOVE) isLoveMuPreference.isChecked = it.getBoolean(LOVE, false)
        }
    }
}